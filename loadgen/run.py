...
# Heavy queries (의도적 지연 — 긴 프롬프트, 복잡한 작업, 멀티에이전트 호출 유도)
HEAVY_QUERIES = [
    # 멀티에이전트 동시 호출 유도
...
def main():
    print(f"Load generator started — target: {ORCHESTRATOR_URL}, interval: {INTERVAL}s")
    ...
    while True:
        ...
        # 지연 콜: heavy는 짧은 간격, normal은 기본 간격
        if qtype == "heavy":
