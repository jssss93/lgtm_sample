...
for agent in orchestrator search summarizer coder; do
  echo "$DAPR_OUT" | grep -q "name: agent-$agent" && pass "agent-$agent 존재" || fail "agent-$agent 미존재"
done
...
echo ""
echo "--- RBAC ---"
echo "$DAPR_OUT" | grep -q "kind: ServiceAccount" && pass "ServiceAccount 존재" || fail "ServiceAccount 미존재"
echo "$DAPR_OUT" | grep -q "kind: Role" && pass "Role 존재" || fail "Role 미존재"
echo "$DAPR_OUT" | grep -q "kind: RoleBinding" && pass "RoleBinding 존재" || fail "RoleBinding 미존재"

# 6. SecurityContext 확인
echo ""
echo "--- SecurityContext ---"
echo "$DAPR_OUT" | grep -q "runAsNonRoot: true" && pass "runAsNonRoot 설정" || fail "runAsNonRoot 미설정"
echo "$DAPR_OUT" | grep -q "allowPrivilegeEscalation: false" && pass "allowPrivilegeEscalation false" || fail "allowPrivilegeEscalation 미설정"
...
echo ""
echo "--- Lifecycle ---"
echo "$DAPR_OUT" | grep -q "preStop" && pass "preStop hook 존재" || fail "preStop hook 미존재"
