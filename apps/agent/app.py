...
from container import build_use_case, get_prompt_manager
...
async def lifespan(app: FastAPI):
    ...
    # Hot-Reload watcher 시작
    pm = get_prompt_manager()
    if pm:
        await pm.start_watcher()
    yield
    # Hot-Reload watcher 종료
    if pm:
...
@app.get("/prompts")
def prompts_status():
    """현재 프롬프트 버전 및 A/B 테스트 상태 조회."""
    pm = get_prompt_manager()
    if pm:
        return {"agent_type": AGENT_TYPE, "prompts": pm.get_info()}
    return {"agent_type": AGENT_TYPE, "prompts": {}, "source": "config.py (no prompt manager)"}
