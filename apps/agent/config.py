...
# 에이전트 타입별 샘플링 비율 (0.0~1.0). 0 = 비활성화.
JUDGE_SAMPLE_RATES: dict[str, float] = {
    "orchestrator": float(os.getenv("JUDGE_SAMPLE_RATE_ORCHESTRATOR", "0.3")),
    "search":       float(os.getenv("JUDGE_SAMPLE_RATE_SEARCH",       "0.1")),
    "summarizer":   float(os.getenv("JUDGE_SAMPLE_RATE_SUMMARIZER",   "0.1")),
    "coder":        float(os.getenv("JUDGE_SAMPLE_RATE_CODER",        "0.2")),
    "default":      float(os.getenv("JUDGE_SAMPLE_RATE_DEFAULT",      "0.1")),
}
