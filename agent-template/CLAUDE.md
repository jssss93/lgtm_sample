...
| `hooks/post_gen_project.sh` | 생성 후 훅: `../agent/`에서 플랫폼 모듈 전체 복사 (아래 목록 참고) + docker-compose 추가 안내 출력 |
...
3. `docker-compose.dapr.yml`에 서비스 + Dapr sidecar 추가 (post_gen 스크립트가 안내)
4. K8s: `deploy/helm/values.yaml`의 agents 배열에 항목 추가
